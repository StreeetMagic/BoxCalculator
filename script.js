...
function drawDimension(group, start, end, options, label) {
  ...
  const labelBaseX = lineStart.x + (lineEnd.x - lineStart.x) * labelAlong;
  const labelBaseY = lineStart.y + (lineEnd.y - lineStart.y) * labelAlong;

  const labelX = labelBaseX + perpX * labelPerp;
  const labelY = labelBaseY + perpY * labelPerp;

  const extensionStart = createSvgElement('line', {
    x1: start.x.toFixed(2),
    y1: start.y.toFixed(2),
    x2: lineStart.x.toFixed(2),
    y2: lineStart.y.toFixed(2),
  ...
  if (labelRotate !== 0) {
    text.setAttribute('transform', `rotate(${labelRotate.toFixed(2)} ${labelX.toFixed(2)} ${labelY.toFixed(2)})`);
  }
